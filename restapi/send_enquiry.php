...
if($senter_id!="")
{
	...
	$receiver_id=$db->escap($receiver_id);
	$cat_id=$db->escap($cat_id);
	$ans=$db->escap($ans);
	$fcm_token=$db->Extract_Single("select fcm_token from register where id='$receiver_id'");
	$ans1=explode("*,*", $ans);
...
$result=$db->que("insert into sent_enquiry set $set");
