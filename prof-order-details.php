...
/*accept and delete*/
$actresponse=isset($actresponse)?$actresponse:'';
$delete=isset($delete)?$delete:'';
if(isset($_REQUEST['actresponse'])){
	$respid=$db->escapstr(base64_decode($actresponse));
	$db->insertrec("update sent_enquiry set prof_response='3' where id='$respid'");
	header("Location: prof-order.php?success");
	echo "<script>href.location='prof-order.php?success'</script>";
	exit;
}
if(isset($_REQUEST['delete'])){
	$delete=base64_decode($_REQUEST['delete']);
	$db->insertrec("update sent_enquiry set prof_response='6' where id='$delete'");
	header("Location: prof-order.php?canceled");
	echo "<script>href.location='prof-order.php?canceled'</script>";
	exit;
}
/*End Accept and delete*/
    ...
    <section class="parallax-window" data-parallax="scroll" data-image-src="img/home_bg_2.jpg" data-natural-width="1400" data-natural-height="470">
               <div class="parallax-content-1">
                  <div class="animated fadeInDown">
					<h1 style="font-size: 35px;">Welcome Professional Name</h1>
				  </div>
		...
		<div class="row mb10">
		   ...
		   <div class="col-sm-12">
			   ...
			   <!--<div class="form-group mb25">
					<a href="prof-order-details.php?actresponse=<?php echo base64_encode($rid); ?>" class="btn_1 outline">Accept</a>
					<a href="prof-order-details.php?delete=<?php echo base64_encode($rid); ?>" onclick="if(confirm('Are you sure want to cancel this order?')) { return true; }else { return false; }" class="btn btn-danger-outline">Cancel</a>
			   </div>-->
		   </div>
		</div>
