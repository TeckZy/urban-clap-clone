...
if($act == "del") {
	...
	echo "<script>location.href='review_userwise.php?id=$id&act=delt'</script>";	
    header("location:review_userwise.php?id=$id&act=delt");
    exit ;
}
if($status == "1") {
    $db->insertrec("update review set active_status='0' where review_id='$review'");
	
	echo "<script>location.href='review_userwise.php?id=$id&act=sts'</script>";
    header("location:review_userwise.php?id=$id&act=sts");
    exit ;
}
else if($status == "0") {
...
$GetRecords=$db->get_all("select * from review where user_id='$id'");
...
foreach($GetRecords as $i=>$GetRecord) {
	$titleName = userInfo($GetRecord['professionalid'],'fname');
    ...
    if($active_status == '0'){
        $DisplayStatus =  "<a href='review-view.php?id=$idvalue' title='View Review Details' title='Activate' class='btn btn-sm btn-icon btn-secondary btn-info'><i class='fa fa-times'></i></a>";
		$Title = "Active";
		$status_active = "Deactive";
	}	
    else if($active_status == '1'){
        $DisplayStatus = "<a href='review-view.php?id=$idvalue' title='View Review Details' title='Deactivate' class='btn btn-sm btn-icon btn-secondary btn-success'><i class='fa fa-check'></i></a>";
		$Title = "Deactive";
    ...
    $disp .="<tr>
				...
				<a href='review-view.php?id=$idvalue' title='View Review Details' class='btn btn-sm btn-icon btn-secondary' data-toggle='tooltip'><i class='fa fa-search'></i></a>
					$DisplayStatus
					<a href='review_userwise.php?id=$id&review=$idvalue&act=del' title='Delete' onclick='return makesure()' type='button' class='btn btn-sm btn-icon btn-secondary btn-danger' title='Delete'><i class='fa fa-trash-o'></i></a>
				</div>
				</td>
			</tr>";
			$sno++;
