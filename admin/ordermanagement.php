...
if($act == "del") {
	...
	echo "<script>location.href='ordermanagement.php?act=delt'</script>";	
    header("location:ordermanagement.php?act=delt");
    exit ;
}
if($status == "1") {
    $db->insertrec("update sent_enquiry set status='0' where id='$id'");
	
	echo "<script>location.href='ordermanagement.php?act=sts'</script>";
    header("location:ordermanagement.php?act=sts");
    exit ;
}
else if($status == "0") {   
...
$GetRecords=$db->get_all("select * from sent_enquiry where role='1' order by id desc");
$sno = 1;
...
foreach($GetRecords as $req){
    ...
    if($active_status == '1'){
        $DisplayStatus = "<a href='ordermanagement.php?id=$idvalue&status=$active_status' title='Activate' class='btn btn-sm btn-icon btn-secondary btn-info'><i class='fa fa-times'></i></a>";
		$Title = "Active";
		$status_active = "Deactive";
	}	
    else if($active_status == '0'){
        $DisplayStatus = "<a href='ordermanagement.php?id=$idvalue&status=$active_status' title='Deactivate' class='btn btn-sm btn-icon btn-secondary btn-success'><i class='fa fa-check'></i></a>";
		$Title = "Deactive";
		$status_active = "Active";
	}
	if($profstat==0){
		$stats="<div  style='color:orange;'><b>Pending</b></div>";
	}else if($profstat==3){
		$stats="<div style='color:blue;'><b>Accept</b></div>";
	}else if($profstat==6){
		$stats="<div style='color:red;'><b>Cancelled</b></div>";
	}else if($profstat==4){
		$stats="<div  style='color:green;'><b>Completed</b></div>";
	}
	
    $name = $username;
	$name="<a href='orderview.php?id=$idvalue' target='_blank'>$GT_RightSign $name</a>";
	$EditLink="";
    ...
    $disp .="<tr>
				...
				<a href='grouporderview.php?id=$idvalue' title='View company Details' class='btn btn-sm btn-icon btn-secondary' data-toggle='tooltip'><i class='fa fa-search'></i></a>
				    $DisplayStatus
					<a href='ordermanagement.php?id=$idvalue&act=del' title='Delete' onclick='return makesure()' type='button' class='btn btn-sm btn-icon btn-secondary btn-danger' title='Delete'><i class='fa fa-trash-o'></i></a>				
				</div>
				</td>
			</tr>";
			$sno++;
      ...
      <div class="content-wrapper">
        <div class="content-header row">
          <div class="content-header-left col-md-6 col-xs-12 mb-2">
            <h3 class="content-header-title mb-0">All Order</h3>
          </div>
        </div>
	        ...
	        <div class="card">
	            <div class="card-header">
				 <h4 class='succ_msg'><?echo $Message;?></h4>
	                <h4 class="card-title">All Ordered List</h4>
