...
foreach($GetRecords as $i=>$GetRecord) {
	...
	$profid = $GetRecord['professionalid'];
	...
	$l_fname = userInfo($profid,'fname');
	$l_lname = userInfo($profid,'lname');
	$lyr_name = $l_fname.' '.$l_lname;
	$u_fname = userInfo($user_id,'fname');
	$u_lname = userInfo($user_id,'lname');
	$user_name = $u_fname.' '.$u_lname;
	
	//$title = userInfo($profid,'title');
	$title="";
    if($active_status == '1'){
	...
	$title="<a target='_blank' href='review.php?id=$profid'>$GT_RightSign $title</a>";
	
    $disp .="<tr>
				<td  align='left'>$sno</td>
				<td  align='left'>$user_name</td>
				<td  align='left' class='name'>$lyr_name</td>
