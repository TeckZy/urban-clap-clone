...
if($act == "apntdelt")
     $Message = "<font color='green'><b>Appointment Deleted Successfully</b></font>" ;
 if($act == "usrdelt")
...
if($act == "usrdel") {
	...
	$db->insertrec("delete from appointment where user_id='$id'");
	 ...
	 {		 
		$sql.="  and";
			...
			$sql1="";
				...
				$divIds = explode(",",$case1['exp_division']);
...
?>
  <script>$(document).ready(function(){
    ...
    <?	 
	 ...
	 $max_date=$db->extract_single("SELECT max(book_date) FROM appointment");
	  $min_date=$db->extract_single("SELECT min(book_date) FROM appointment");
	if (($lawyername != '')|| ($username != '')|| ($from1 !="") ||($to1 != "") || ($case != ""))
	 {		 
		$sql.=" where";
		 
			if((isset($lawyername)) && ($lawyername != ''))
			{
			$lawyer=$db->get_all("select * from register where user_role='1' and fname like '%$lawyername%' ");
			$sql1="";
					   ...
					   if ($sql2 != '' )
		   {
			...
			$to1=$max_date;	
	            ...
	            $sql.= " book_date between '".$from3."' and '".$to3."'";
...
foreach($GetRecords as $i=>$GetRecord) {
    ...
    if($approve_status == '0'){
		...
		$status_active = "Active";
	             ...
	             if ($approve_status== 0) { 
				 $dip = "<label class='btn-sm btn-warning'>In progress <i class=' icon-ok-circled-1'></i></label>";}
				  else if ($approve_status == 1){ 
								$dip = "<label class='btn-sm btn-success'>Accepted <i class=' icon-ok-circled-1'></i></label>";
							    } else if ($approve_status== 2) { 
							    $dip = "<label class='btn-sm btn-danger'>Cancelled <i class='icon-cancel-circled'></i></label>";
							    } 
    $disp .="<tr>
				<td  align='left'>$sno</td>
				<td  align='left' class='name'><a href='userview.php?id=$user_id' target='_blank'>".$GT_RightSign."".$com_obj->get_name($user_id)."</a></td>
				<td  align='left'><a href='company-view.php?id=$lawyer_id' target='_blank'>".$GT_RightSign."".$com_obj->get_name($lawyer_id)."</a></td>
